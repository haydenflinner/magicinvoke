...
def clean(ctx, cfiles, objectfiles, executable_path, dry_run=False):
    removing = ' '.join(str(p) for p in itertools.chain(cfiles, objectfiles, [executable_path]))
    if dry_run:
        print(removing)
        return
...
def mycompile(ctx, cfiles, objectfiles: [OutputPath]):
    [ctx.run("gcc -c {} -o {}".format(c, o)) for c, o, in zip(cfiles,objectfiles)]
...
def link(ctx, objectfiles: [InputPath], executable_path: OutputPath):
    ctx.run("gcc -o {} {}".format(executable_path, ' '.join(str(f) for f in objectfiles)))
