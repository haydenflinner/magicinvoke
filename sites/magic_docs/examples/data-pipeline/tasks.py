...
def test(ctx):
    ...
    print("Assuming a clean slate, everything should run from scratch.")
    both_ran(ctx.run("invoke print-peoples-ages").stdout)
    only_print_ran(ctx.run("invoke print-peoples-ages").stdout)

    print("Since latest outputs were generated by a task with different flags, "
          "everything should run again when flags change.")
    both_ran(
        ctx.run(
    ...
    print("Flags changed _again_!")
    both_ran(ctx.run("invoke print-peoples-ages").stdout)

    print("Make sure clean actually cleans.")
